...
assign("EIF4G1_CNV_COR_RNAseq_sig", data.table::fread(
  file.path(
    output_directory,
    "ProcessedData",
...
assign("EIF4A2_CNV_COR_RNAseq_sig", data.table::fread(
  file.path(
    output_directory,
    "ProcessedData",
...
assign("EIF3E_CNV_COR_RNAseq_sig", data.table::fread(
  file.path(
    output_directory,
    "ProcessedData",
...
assign("EIF3H_CNV_COR_RNAseq_sig", data.table::fread(
  file.path(
    output_directory,
    "ProcessedData",
...
plot_heatmap <- function(df, gene) {
  gene_ht <- ComplexHeatmap::Heatmap(
    as.matrix(df),
    ...
    column_title = paste(gene, "differential correlation"),
    column_title_gp = grid::gpar(fontsize = 15, fontface = "bold"),
    column_names_gp = grid::gpar(fontsize = 12, fontface = "bold"),
    column_names_rot = 0,
    show_row_names = FALSE,
    row_names_gp = grid::gpar(fontsize = 6),
    # Heatmap() internally calls kmeans() with random start points, 
    # which results in, for some cases, generating different clusters from repeated runs. 
  ...
  pdf(file.path(
    path = file.path(output_directory, "Fig2"),
    filename = paste(gene, "CNV CORs heatmap.pdf")
  ),
  width = 8,
  height = 10,
  useDingbats = FALSE
...
.get_cluster_genes <- function(df1, df2) {
  #gene_ht1 <- ComplexHeatmap::draw(df2,
  #                                 merge_legends = TRUE,
  #                                 heatmap_legend_side = "bottom",
  #                                 annotation_legend_side = "bottom")
  cluster.geneID.list <- function(cluster_label) {
    ...
    write.csv(c1, file= file.path(output_directory,"ProcessedData", 
                                  paste0("CNV_RNAseq_COR_cluster", cluster_label,".csv")),
              row.names=FALSE)
...
.pathway_dotplot <- function(df, gene_name, pathway) {
  ...
  ggplot2::ggsave(
    path = file.path(output_directory, "Fig2"),
    filename = paste(gene_name, " pathway cluster",".pdf"),
    plot = p1,
    width = 10,
    height = 10,
    useDingbats = FALSE
...
.plot_cluster_gene_network <- function(data, cluster_number, gene_name) {
  p2 <- enrichplot::cnetplot(ReactomePA::enrichPathway(
    ...
    ggtitle(paste(gene_name, "cluster", cluster_number)) +
    theme(plot.title = element_text(hjust = 0.5))
  print(p2)
  
  ggplot2::ggsave(
    path = file.path(output_directory, "Fig2"),
    filename = paste(gene_name, "COR","cluster", cluster_number, "cnet",".pdf"),
    plot = p2,
    width = 9,
    height = 9,
    useDingbats = FALSE
...
COR_analysis <- function(df, gene) {
  gene_ht <- plot_heatmap(df, gene)
...
COR_analysis(df = EIF4G1_CNV_COR_RNAseq_sig, gene = "EIF4G1")

COR_analysis(df = EIF4A2_CNV_COR_RNAseq_sig, gene = "EIF4A2")
