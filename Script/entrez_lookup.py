...
For example, one way of mapping gene symbols to Entrez IDs is to read a list of entries from a local flat file such as
'Hs.data'.  Another way is to make a symbol-specific call to a remote Entrez database.

The purpose of this module is to "hide" from the rest of the package the particular implementation for Entrez lookups,
so that it may be updated or changed without impacting any other code.
"""
...
class EntrezLookup:
  ...
  """
  # TODO(dlroxe): Hmm, looks like 'Hs.data' started going stale around 2013.  Is there another source (that doesn't
  ...
  def __init__(self, hs_file: Optional[str]):
    # It may be useful in tests to short-circuit Entrez lookups to immediately return 'None'.
  ...
  def translate_gene_symbol_to_entrez_id(self, gene_symbol):
    ...
    return self._hs_data_dict[key].gene_id if key in self._hs_data_dict else None
      ...
      try:
        ...
        while True:
          try:
            ...
            logging.warning('error parsing record %s (previous well-parsed ID: %s):\n%s', count, last_id, ex)
            break
